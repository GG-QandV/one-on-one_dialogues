...
class VadConfig:
    ...
    onset_offset_db: float = 2.0
    #: Порог выхода. Ниже порога входа — это и есть гистерезис.
    #: Обязан превышать разброс кадрового уровня шума (доли dB), иначе
    #: шумовые пики в паузе снова «зажигают» SPEECH, таймер тишины
    #: сбрасывается и реплики склеиваются до принудительной резки. 0.8 dB
    #: было ниже этого разброса и давало max_length-склейки.
    release_offset_db: float = 1.5
...
class VadDetector:
    ...
    def _adapt_noise(self, level: float) -> None:
        ...
        if level > self._noise_db:
            ...
            # Наблюдаемый уровень — нижняя граница оценки: счётчик обновляется
            # только по подпороговым кадрам, поэтому без этого ограничения он
            # «сползает» ниже самого шума, порог выхода оказывается на уровне
            # шума, и тишина перестаёт детектироваться.
            self._noise_db = max(
                self._cfg.absolute_floor_db,
                self._noise_db - step_down,
                level,
            )
